        ...
        _consumerTask = Task.Run(async () =>
        {
            try
            {
                while (!_cancellationTokenSource.Token.IsCancellationRequested)
                {
                    try
                    {
                        ConsumeResult<string, string> consumeResult = _consumer.Consume(_cancellationTokenSource.Token);
        ...
        try
        {
            switch (consumeResult.Topic)
            {
                ...
                case KafkaTopics.OrderStatus:
                    OrderStatusDto statusUpdate = JsonSerializer.Deserialize<OrderStatusDto>(consumeResult.Message.Value,
                        new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
